...
MY_PASS = os.environ.get("MY_PASS",'A2Z123')
pass_dict = {}
...
async def login_handler(c: Client, m: Message):
    try:
        try:
            ag = await m.reply_text("Now send me password.\n\n for password send 10rs to `chinmaysah00@ybl` and send screen shot to @A2Z_movies_bot \n\n (for running bot need contributions..unwanted message = ban + report)")
...
async def channel_receive_handler(bot, broadcast):
    if MY_PASS:
        ...
        if check_pass == None:
            await broadcast.reply_text("Login first using /login cmd \n don\'t know the pass? request it from @A2Z_movies_bot")
            return
        if check_pass != MY_PASS:
